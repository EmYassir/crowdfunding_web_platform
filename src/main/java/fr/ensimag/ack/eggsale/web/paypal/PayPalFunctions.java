...
public class PayPalFunctions  {
    ...
    public PayPalFunctions() {//lhuynh - Actions to be Done on init of this class
        ...
        InputStream inStream = PayPalFunctions.class.getClassLoader().getResourceAsStream("paypal.properties");
        try {
            prop.load(inStream);
            inStream.close();
        } catch (IOException ex) {
            Logger.getLogger(PayPalFunctions.class.getName()).log(Level.SEVERE, null, ex);
        }
        
        returnURL = prop.getProperty("returnURL"); 
        cancelURL = prop.getProperty("cancelURL"); 
        paypalURL = prop.getProperty("paypalURL"); 
    ...
    gv_APIUserName	= prop.getProperty("api.username");
    gv_APIPassword	= prop.getProperty("api.password");
    gv_APISignature = prop.getProperty("api.signature");
...
public HashMap CallShortcutExpressCheckout( String paymentAmount)
{
	...
	*/
    ...
    is stored in $nvpstr
    */
    String nvpstr = "&PAYMENTREQUEST_0_AMT=" 
            + paymentAmount 
            + "&PAYMENTREQUEST_0_PAYMENTACTION=" 
            + paymentType + "&ReturnUrl=" 
...
/*********************************************************************************
  ...
  * Output: Returns a HashMap object containing the response from the server.
*********************************************************************************/
public HashMap CallMarkExpressCheckout( String paymentAmount, String shipToName, String 										shipToStreet, String shipToCity, String shipToState,
